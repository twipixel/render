...
const regJS = /\.js$/
  ...
  , regDotFolder = /^\./
  , regMin = /\.min\.js/gi
  , regModules = /modules$/i
  , regAssets = /assets/i
  , regBundle = /\.bundle\.js/gi
  , PATH_SRC = './src/'
  , PATH_PIXI = './pixi/'
  , PATH_TEST = './test/'
  , PATH_DIST = './dist/'
  , PATH_ASSET = './test/assets/'
  , PATH_EXTERNAL_LIB = './external/lib/'
  , PATH_EXTERNAL_VENDOR = './external/vendor/'
  , PATH_NODE_MODULES = './node_modules/';

const math = fs.readdirSync('./test/math', 'utf8').map(folder => ({path: './test/math/', folder}));
const pixi = fs.readdirSync('./test/pixi', 'utf8').map(folder => ({path: './test/pixi/', folder}));
const webgl = fs.readdirSync('./test/webgl', 'utf8').map(folder => ({path: './test/webgl/', folder}));
const webglFundamentals = fs.readdirSync('./test/webgl-fundamentals', 'utf8').map(folder => ({path: './test/webgl-fundamentals/', folder}));
const entries = math.concat(pixi, webgl, webglFundamentals);
...
const setEntry = entries => {
  ...
  var result = entries.reduce((entriesObject, {path, folder}) => {
    folder = folder.replace(regJS, '');
    entriesObject[folder] = path + folder + '/index.js';
